package de.bht.consilio.anim
{
	...
	public class AnimatedSprite extends IsoObject
	{
		...
		public function init():void {
			_pieceDescription = SpriteSheets.getInstance().descriptionForName(name);
			
			_animationOffsetContainer.x = - _pieceDescription.xoffset;
			_animationOffsetContainer.y = - _pieceDescription.yoffset;
			
			var frames:Array = SpriteSheets.getInstance().animationDataForName(name).frames as Array;
			
			_animationData = createAnimationData(frames);
			
			if(!(spriteSheets.hasOwnProperty(name))) {
				spriteSheets[name] = SpriteSheets.getInstance().sheetForName(name).bitmapData;
			}
...
//			trace("Memory used before accessing sheet: " + Number( System.totalMemory / 1024 / 1024 ).toFixed( 2 ) + "Mb");
			...
			_picture.bitmapData.copyPixels(spriteSheets[name], new Rectangle(f.x, f.y, f.w, f.h), new Point(0, 0));
